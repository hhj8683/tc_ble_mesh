...
void proc_suspend_low_power()
{
    ...
    if(clock_time_exceed(mesh_sleep_time.last_tick, (GET_ADV_INTERVAL_MS(blta.advInt_min) - 10) * 1000)){
        mesh_sleep_time.last_tick = clock_time() | 1;
        bls_pm_setSuspendMask (SUSPEND_DISABLE);
        app_enable_scan_all_device();   // enable scan mode.
        mesh_send_adv2scan_mode(0);     // enter scan immediately.
    }
...
//	}else if (blc_ll_getCurrentState() == BLS_LINK_STATE_ADV)
    {
		if(clock_time_exceed(mesh_sleep_time.last_tick, mesh_sleep_time.run_time_us)){
			...
			if(!is_provision_working() ){
                blc_ll_setScanEnable (0, 0); // scan is unnecessary during early wakeup, disable to save power.
