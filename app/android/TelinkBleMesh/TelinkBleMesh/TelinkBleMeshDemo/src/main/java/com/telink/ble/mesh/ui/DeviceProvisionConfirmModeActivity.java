/********************************************************************************************************
 * @file DeviceProvisionConfirmModeActivity.java
 *
 ...
 */
public class DeviceProvisionConfirmModeActivity extends BaseActivity implements View.OnClickListener, EventListener<String> {
     ...
     * data adapter
     */
    private DeviceConfirmProvisionListAdapter mListAdapter;
    ...
    private void initView() {
        setContentView(R.layout.activity_device_provision_confirm);
        initTitle();
        RecyclerView rv_devices = findViewById(R.id.rv_devices);
        devices = new ArrayList<>();

        mListAdapter = new DeviceConfirmProvisionListAdapter(this, devices);
...
//        MeshLogger.d("device uuid calc by md5  - " + Arrays.bytesToHexString(Encipher.calcUuidByMac(advertisingDevice.device.getAddress())));
    ...
    private void startProvision(NetworkingDevice processingDevice) {
        ...
        processingDevice.addLog(NetworkingDevice.TAG_PROVISION, "continue provision -> 0x" + String.format("%04X", address));
    ...
    public void identifyDevice(int position) {
        if (isScanning) {
            isScanning = false;
            MeshService.getInstance().stopScan();
        }
        ...
        ProvisioningDevice provisioningDevice = new ProvisioningDevice(processingDevice.bluetoothDevice, processingDevice.nodeInfo.deviceUUID);
        ...
        provisioningDevice.setAttentionDuration((byte) time);
//        processingDevice.state = NetworkingState.PROVISIONING;
        processingDevice.addLog(NetworkingDevice.TAG_PROVISION, "action start -> provision identity");
        mListAdapter.setIdentifyingPosition(position);
