/********************************************************************************************************
 * @file     SDKLibCommand+subnetBridge.m
 *
...
#import "SDKLibCommand+SubnetBridge.h"

@implementation SDKLibCommand (subnetBridge)
...
+ (SigMessageHandle *)bridgingTableSizeGetWithDestination:(UInt16)destination retryCount:(NSInteger)retryCount responseMaxCount:(NSInteger)responseMaxCount successCallback:(responseBridgingTableSizeStatusMessageBlock)successCallback resultCallback:(resultBlock)resultCallback {
    SigBridgingTableSizeGet *message = [[SigBridgingTableSizeGet alloc] init];
    SDKLibCommand *command = [[SDKLibCommand alloc] initWithMessage:message retryCount:retryCount responseMaxCount:responseMaxCount responseAllMessageCallBack:(responseAllMessageBlock)successCallback resultCallback:resultCallback];
    command.responseBridgingTableSizeStatusCallBack = successCallback;
    return [SigMeshLib.share sendConfigMessage:message toDestination:destination command:command];
}
