 ...
 */
#if (PM_DEEPSLEEP_RETENTION_ENABLE) // BLE_SRC_TELINK_MESH_EN
_attribute_ram_code_	// add by BLE Team
#endif
void gpio_init(int anaRes_init_en) // BLE_SRC_TELINK_MESH_EN
{
...
static void gpio_set_mux(GPIO_PinTypeDef pin, GPIO_FuncTypeDef func)
{
	...
	switch(pin)
	{
		case GPIO_PA0:
		{
			...
			}else if((func == AS_PWM0_N) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
				val = BIT(0);
			}
		...
		case GPIO_PA2:
		{
			...
			else if((func == AS_PWM0) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
				val = BIT(5);
			}
		}
		break;
		...
		case GPIO_PA3:
		{
			...
			else if((func == AS_PWM1) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
				val = BIT(7);
			}
		}
		break;
		...
		case GPIO_PA4:
		{
			...
			else if((func == AS_PWM2) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
				val = BIT(1);
			}
		}
		break;
		...
		case GPIO_PB0:
		{
			...
			//2. ATSEL1
			mask = (unsigned char)~(BIT(1)|BIT(0));
			if((func == AS_PWM3) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
		...
		case GPIO_PB1:
		{
			...
			//2. ATSEL2
			mask = (unsigned char)~(BIT(3)|BIT(2));
			if((func == AS_PWM4) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
		...
		case GPIO_PB2:
		{
			...
			//2. RX_CYC2LNA
			mask = (unsigned char)~(BIT(5)|BIT(4));
			if((func == AS_PWM5) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
...
//			}
		...
		case GPIO_PB3:
		{
			...
			//2. TX_CYC2PA
			mask = (unsigned char)~(BIT(7)|BIT(6));
			if((func == AS_PWM0_N) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
		...
		case GPIO_PB4:
		{
			...
			}else if((func == AS_PWM4) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
				val = BIT(0);
			}
			else if(func == AS_CMP){
				val = BIT(1);
		...
		case GPIO_PB5:
		{
			...
			}else if((func == AS_PWM5) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
				val = BIT(2);
			}
			else if(func == AS_CMP){
				val = BIT(3);
		...
		case GPIO_PC0:
		{
			...
			}else if((func == AS_PWM4_N) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
				val = BIT(0);
			}
			else if(func == AS_UART){
				val = BIT(1);
		...
		case GPIO_PC1:
		{
			...
			mask = (unsigned char)~(BIT(3)|BIT(2));
			if(func == AS_I2C){
			}else if((func == AS_PWM1_N) || (func == AS_PWM_SECOND)){ // BLE_SRC_TELINK_MESH_EN
				val = BIT(2);
			}
			else if((func == AS_PWM0) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
				val = BIT(3);
		...
		case GPIO_PC2:
		{
			...
			//2. I2C_SDA
			mask = (unsigned char)~(BIT(5)|BIT(4));
			if((func == AS_PWM0) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
		...
		case GPIO_PC3:
		{
			...
			//2. I2C_SCK
			mask = (unsigned char)~(BIT(7)|BIT(6));
			if((func == AS_PWM1) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
		...
		case GPIO_PC4:
		{
			...
			//2. PWM0_N
			mask = (unsigned char)~(BIT(1)|BIT(0));
			if((func == AS_PWM2) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN

			}else if(func == AS_UART){
				val = BIT(0);
			}
			else if((func == AS_PWM0_N) || (func == AS_PWM_SECOND)){ // BLE_SRC_TELINK_MESH_EN
				val = BIT(1);
			}
		}
		break;
		...
		case GPIO_PC5:
		{
			...
			//2. ATSEL_0
			mask = (unsigned char)~(BIT(3)|BIT(2));
			if((func == AS_PWM3_N) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
		...
		case GPIO_PC7:
		{
			...
			else if((func == AS_PWM5_N) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
				val = BIT(7);
			}
			else if(func==AS_TX_CYC2PA){
				val =0;
		...
		case GPIO_PD2:
		{
			...
			else if((func == AS_PWM3) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
				val = BIT(5);
			}
		}
		break;
		...
		case GPIO_PD3:
		{
			...
			//2. UART_TX7816
			mask = (unsigned char)~(BIT(7)|BIT(6));
			if((func == AS_PWM1_N) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
		...
		case GPIO_PD4:
		{
			...
			else if((func == AS_PWM2_N) || (func == AS_PWM)){ // BLE_SRC_TELINK_MESH_EN
				val = BIT(1);
			}
		}
		break;
