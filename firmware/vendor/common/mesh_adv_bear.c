...
/**
 * @brief       This function servers to check whether the opcode is mesh ota opcode
 * @param[in]   op	- opcode
 * @return      1: is mesh ota opcode   0: not mesh ota opcode
 * @note        
 */
int is_mesh_ota_opcode(u16 op)
{
    ...
    if((FW_UPDATE_START == op) || (BLOB_CHUNK_TRANSFER == op) || (BLOB_BLOCK_STATUS == op) || (BLOB_PARTIAL_BLOCK_REPORT == op) ||
       (FW_DISTRIBUT_STATUS == op) || (BLOB_INFO_STATUS == op)){
        return 1;
    }
...
int is_not_use_extend_adv(u16 op)
{
	...
	#endif
    || is_mesh_ota_opcode(op)  // TODO : only Chunk data now.
    ){
...
u8 mesh_blc_ll_setExtAdvParamAndEnable(void)
{
...
#if BLE_MULTIPLE_CONNECTION_ENABLE
    status = blc_ll_setExtAdvParam( GATT_ADV_HANDLE,         ADV_EVT_PROP_LEGACY_CONNECTABLE_SCANNABLE_UNDIRECTED,  ADV_INTERVAL_MS,           ADV_INTERVAL_MS + ADV_INTERVAL_RANDOM_MS,
    ...
    // not need to enable in single connection sdk to save code size
    //status = blc_ll_setExtAdvEnable_1( BLC_ADV_ENABLE, 1, ADV_HANDLE0, 0 , 0);
#endif
 ...
 *              no dma length in raw_pkt here(dma length's position is (raw_pkt-4)).
 */
...
#if LOG_UNEXPECTED_EXT_ADV_EN
    mesh_unexpected_ext_adv_filter(raw_pkt);
#endif
...
static inline int send_adv_every_prepare_cb(void)
{
    ...
    if(GET_ADV_INTERVAL_MS(blc_ll_getAdvInterval()) > ADV_INTERVAL_MS/2){
        ret = 1;
    }
#else
...
int mesh_send_adv2scan_mode(int tx_adv)
{
    ...
    #endif
#else
	if((0 == tx_adv) || (BLS_LINK_STATE_ADV == blc_ll_getCurrentState()) || ((BLS_LINK_STATE_CONN == blc_ll_getCurrentState()) && (!is_in_brx_window()) && rf_link_time_allow(EXTENDED_ADV_ENABLE?5000:3000))){
...
_attribute_ram_code_ void adv_homekit_filter(u8 *raw_pkt)
{
	...
	if((!blt_pair_ok) && !is_in_brx_window()){// homekit pair proc in irq
...
#if((MCU_CORE_TYPE == MCU_CORE_8258) || (MCU_CORE_TYPE == MCU_CORE_8278) || (MCU_CORE_TYPE == MCU_CORE_TC321X))
...
_attribute_ram_code_ u8 adv_filter_proc(u8 *raw_pkt ,u8 blt_sts)
{
	...
	#if MESH_MONITOR_EN
	if(!((blc_ll_getCurrentState() == BLS_LINK_STATE_CONN) && is_in_brx_window())){
