...
/* process in proc_led()
 * on: LED on duration. unit: 64ms
 * off: LED off duration. unit: 64ms
 * n: LED on count
 * sel: bit of LED
 * led shining total time = n * (on + off) * 64ms
 */
...
#define	LED_EVENT_FLASH_1HZ_START(n)		config_led_event(8,8,n,LED_MASK)
...
#define	LGT_CMD_RESET_NETWORK        	0xca
