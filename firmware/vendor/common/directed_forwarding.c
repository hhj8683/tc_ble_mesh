		...
		foreach(i, MAX_NON_FIXED_PATH){
            ...
            if(g_non_fixed_entry.p_state->lifetime_ms){
    			...
    			if(is_addr_in_entry(src_address, destination, g_non_fixed_entry.p_entry)){
    				if(g_non_fixed_entry.p_entry->path_not_ready){
    					return 0;
    				}
    				else{
    					return &g_non_fixed_entry;
...
int update_discovery_entry_by_path_request(discovery_entry_t *p_dsc_entry, u16 network_src, u8 src_type, mesh_ctl_path_req_t *p_path_request, non_fixed_entry_t *p_fwd_entry)
{
    int update_flag = 0;
    if(p_fwd_entry && p_fwd_entry->p_state->path_metric_zero_exist){ // only path target may meet condition
        if(p_path_request->origin_path_metric){
            if(p_dsc_entry->path_metric == 0){
                update_flag = 1;
            }
            else if(p_path_request->origin_path_metric < p_dsc_entry->path_metric){
                update_flag = 1;
            }
        }
    }
...
discovery_entry_par_t * add_discovery_table_entry(u16 netkey_offset, addr_range_t *p_path_origin, u16 destination, addr_range_t *p_dependent_origin, u16 next_toward_path_origin, u8 bearer_toward_path_origin, mesh_ctl_path_req_t *p_path_req)
{
	...
	foreach(i, MAX_DSC_TBL){
		if(0 == discovery_table[netkey_offset].dsc_entry_par[i].entry.path_origin.addr){
			...
			LOG_DF_DEBUG(0, 0, "add discovery_table origin:0x%x target:0x%x fwd_num:%d", dsc_tbl_entry.path_origin.addr, dsc_tbl_entry.destination, dsc_tbl_entry.forwarding_number);
...
non_fixed_entry_t * add_new_path_in_non_fixed_fwd_tbl(u16 netkey_offset, non_fixed_entry_t *p)
{
	...
	if(path_index < MAX_NON_FIXED_PATH){
        ...
        LOG_DF_DEBUG(0, 0, "%s origin:0x%x target:0x%x fwd_num:%d update_id:0x%x", __func__, p->p_entry->path_origin, p->p_entry->destination, p->p_state->forwarding_number, model_sig_g_df_sbr_cfg.df_cfg.directed_forward.subnet_state[netkey_offset].update_id);
	}
...
void update_forwarding_entry_by_path_reply_delay_timer_expired(u16 netkey_offset, non_fixed_entry_t *p_fwd_entry, discovery_entry_par_t *p_dsc_entry)
{
    ...
    LOG_DF_DEBUG(0, 0, "%s origin:0x%x target:0x%x lanes:%d update_id:0x%x", __func__, p_fwd_entry->p_entry->path_origin, p_fwd_entry->p_entry->destination, p_fwd_entry->p_state->lane_counter, model_sig_g_df_sbr_cfg.df_cfg.directed_forward.subnet_state[netkey_offset].update_id);
}
...
void update_forwarding_entry_by_path_confirm(u16 netkey_offset, non_fixed_entry_t *p_fwd_entry, mesh_ctl_path_confirmation_t *p_path_confirm)
{
    ...
    LOG_DF_DEBUG(0, 0, "%s origin:0x%x target:0x%x update_id:0x%x", __func__, p_fwd_entry->p_entry->path_origin, p_fwd_entry->p_entry->destination, model_sig_g_df_sbr_cfg.df_cfg.directed_forward.subnet_state[netkey_offset].update_id);
}
...
int prepare_and_send_path_reply(u16 netkey_offset, discovery_entry_par_t *p_dsc_entry, u8 confirm_req)
{
    ...
    int is_target = is_path_target(p_dsc_entry->entry.path_origin.addr, p_dsc_entry->entry.destination);
    if(is_target && (p_dsc_entry->entry.path_metric == 0)){
        p_fwd_entry->p_state->path_metric_zero_exist = 1;
    }
...
void directed_forwarding_pending_check(u16 netkey_offset)
{
	...
	if(p_dsc_pending && (num < model_sig_g_df_sbr_cfg.df_cfg.directed_forward.subnet_state[netkey_offset].max_concurrent_init)){
        start_path_discovery_timer(p_dsc_pending);
					...
					if(1){// dorwarding number always 0 after powerup now.
						...
						if(is_path_target(origin_addr, p_path_req->destination) ||(DIRECTED_RELAY_ENABLE == model_sig_g_df_sbr_cfg.df_cfg.directed_forward.subnet_state[netkey_offset].directed_control.directed_relay)){
							...
							if(!p_dsc_entry){
								...
								if(update_discovery_entry_by_path_request(&p_dsc_entry->entry, p_nw->src, src_type, p_path_req, p_fwd_entry)){
									start_path_request = 1;
								}					
							}
