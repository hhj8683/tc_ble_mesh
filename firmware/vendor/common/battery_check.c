...
#endif
_attribute_no_retention_bss_ u8      adc_hw_initialized = 0;
...
//	when MCU powered up or wakeup from deep/deep with retention, adc need be initialized
	if(!adc_hw_initialized){
		...
		adc_hw_initialized = 1;
            ...
            adc_power_on_sar_adc(1); // because adc was power off before leaving this function
        }
...
//	tlkapi_printf(APP_BATT_CHECK_LOG_EN, "[APP][BAT] The battery power is %dmV!\n", batt_vol_mv);
    LOG_BATTERY_CHECK_DEBUG(0,0,"battery, loop:%d, current:%d, thres:%d", loop_flag, batt_vol_mv, alram_vol_mv);
...
_attribute_no_inline_ void battery_power_low_handle(int loop_flag)
{
	...
	#else
    cpu_sleep_wakeup(DEEPSLEEP_MODE, PM_WAKEUP_TIMER, clock_time() + 50*CLOCK_SYS_TIMER_CLK_1MS);  //
    #endif
    #endif
}
...
{
    u16 alarm_thres = VBAT_DEEP_THRES_MV;
    if(loop_flag){
		...
		|| pm_is_deepPadWakeup() // 32k rc not run without PM_WAKEUP_TIMER  // 
			...
			#if(__PROJECT_MESH_SWITCH__)
			pm_clear_deepPadWakeup();  // clear to avoid run every loop.
			#endif
    	    #if __PROJECT_BOOTLOADER__
